...
- メール内の「Verify Email Address」リンクをクリックすると、出勤前画面が表示されれば認証成功です。  
...
**Mailtrapによるメール認証の挙動を体験できるように、アカウントごとに設定を変えています。**

| 区分            | 名前   | メールアドレス              | パスワード   | メール認証状況                     |
|-----------------|--------|------------------------------|--------------|----------------------------------|
| 一般ユーザー１   | user1  | general1@example.com         | password1    | ✖ 未認証（Mailtrapで認証体験用）  |
| 一般ユーザー２   | user2  | general2@example.com         | password2    | ✔ 認証済み（すぐにログイン可）     |
| 管理者ユーザー   | admin1 | admin1@example.com           | adminpass    | 対象外（管理者は認証不要）         |

> ※「一般ユーザー１」でログインを試みると、メール認証画面にリダイレクトされます。Mailtrapに届いた認証メール内のリンクをクリックして認証を完了してください。

> ※「一般ユーザー２」は認証済みの状態です。すぐにログインし、機能を確認できます。
